...
Помогал команде разработки адпатироваться в Unreal Engine после Unity.
...
В результате мы не только получили классные тени от персонажа как от первого лица, так и в VR. А еще и сделали интересную "отложенную телепротацию", когда игрок видит как аватар двигается к следущей точке маршрута.
Что бы тени VR аватара соотвествовали движениям игрока реализовал FullBody IK.
...
Плагин позволил сократить время разработки 3 проектов упростив сборку механик и миниигр мне и команде.
...
В процессе оптимизации сети перестроил архитектуру проекта для распределения нагрузи сети по времени.
...
Обучал тестировщиков и команду использовать багтреккер.
...
#### Core
Адапатирова и расширил системы сохранений, ачивментов(Steam, EGS, VKplay), алгоритм "квестовой цепочки", логику подключения игроков по сети(Steam, EGS, VKplay).

#### UI
С UI дизайнером делали верстку всех интерфейсов. 
...
#### Core-gameplay
С нуля разработал систему инвентаря с механиками применения предметов друг на друга а-ля "алхимия" с возможностью взаимодействимя предметов как в UI, так и в 3д мире.
Занимался реализацией боевых механик: стрельба, ближний бой, уклонение, а также загадок и миниигр.

#### Multiplayer network
Настроил базу репликации проекта.
Занимался обучением команды правилам репликации в UnrealEngine.
Занимался переработкой архитектуры проекта для реализации нагружденных мест.

#### EpicOnlineServices
Реализовал кроссплей между копиями игры, купленными в разных магазинах. Занимался настройкой ачивментов и сбора статистики.
...
#### Multiplayer network
Углубил понимание netwrok framework.
Настроил собрку dedicated сервера на linux и деплой через докер.
Реализловал систему взаимодействия dedicated сервера, клиентов и матчмейкинга для безопасного получения айтемов игроков, подсчета очков и передачи их в блокчейн.
...
Была задача придумать и постримить разработку увелкательного прототипа для 2 открытых уроков онлайн школы OTUS.
...
Настраивал балланс кор геймплея.
Разработал прототип нового поведения камеры. Нужно было запретить камере показывать игроку геометрию за картой, реализовать механику с минимальными архитектурными изменениями в коде и левелдизайне.
